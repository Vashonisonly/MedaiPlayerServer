...
private:
    ...
    class Client : public BnMediaPlayer {
        ...
        // MStar Android Patch Begin
        virtual status_t        setDataSource(const sp<IMstarMMSource>&Source);
        // MStar Android Patch End
        ...
        // MStar Android Patch Begin
        virtual bool            setPlayMode(int speed);
        virtual int             getPlayMode();
        virtual status_t        setPlayModePermille(int speed_permille);
        virtual int             getPlayModePermille();
        virtual status_t        setAudioTrack(int track);
        virtual status_t        setSubtitleTrack(int track);
        virtual status_t        setSubtitleDataSource(const char *url);
        virtual status_t        offSubtitleTrack();
        virtual status_t        onSubtitleTrack();
        virtual char*           getSubtitleData(int *size);
        virtual char*           getAudioTrackStringData(int *size, int infoType);
        virtual status_t        setSubtitleSurface(const sp<IGraphicBufferProducer>& surface);
        virtual int             setSubtitleSync(int timeMs);
        virtual void            divx_SetTitle(uint32_t u32ID);
        virtual status_t        divx_GetTitle(uint32_t *pu32ID);
        virtual void            divx_SetEdition(uint32_t u32ID);
        virtual status_t        divx_GetEdition(uint32_t *pu32ID);
        virtual void            divx_SetChapter(uint32_t u32ID);
        virtual status_t        divx_GetChapter(uint32_t *pu32ID);
        virtual void            divx_SetAutochapter(uint32_t u32ID);
        virtual status_t        divx_GetAutochapter(uint32_t *pu32ID);
        virtual status_t        divx_GetAutochapterTime(uint32_t u32ID, uint32_t *pu32IdTime);
        virtual status_t        divx_GetChapterTime(uint32_t u32ID, uint32_t *pu32IdTime);
        virtual status_t        divx_SetResumePlay(const Parcel &inResumeInfo);
        virtual status_t        divx_SetReplayFlag(bool isResumePlay);
        virtual status_t        captureMovieThumbnail(sp<IMemory>& mem, Parcel *pstCaptureParam, int* pCapBufPitch);

        virtual status_t        setVideoDisplayAspectRatio(int eRatio);
        virtual status_t        getVideoPTS(int *msec);
        virtual status_t        setAVCodecChanged(bool bChanged);
        // MStar Android Patch End
    ...
    private:
        ...
        mutable     Mutex                       mLock;
                    ...
                    // MStar Android Patch Begin
                    bool                        mIsMstplayer;
                    static int32_t              mMainClientId;
                    bool                        mIsNWayPlayback;
                    int32_t                     mGetkeyvalue;
                    bool                        mEnableMultiDecode;
                    // MStar Android Patch End
