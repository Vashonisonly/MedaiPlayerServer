...
void __attribute__((weak)) Mdrv_SYS_VR360_Set_BW_Setting(void);
void __attribute__((weak)) Mdrv_SYS_VR360_Restore_BW_Setting(void);
#define IS_VR360_SUPPORTED (Mdrv_SYS_VR360_Set_BW_Setting && Mdrv_SYS_VR360_Restore_BW_Setting)
...
void MediaPlayerService::instantiate() {
    ...
    property_set("mstar.media.vdec.reset", "1");
    property_set("mstar.media.adec.reset", "1");
    // MStar Android Patch End
}
...
// MStar Android Patch Begin
int32_t MediaPlayerService::Client::mMainClientId = 0;
// MStar Android Patch End
...
{
    ALOGD("Client(%d) constructor  pid = %d", connId, pid);
    mPid = pid;
    mConnId = connId;
    mService = service;
    mClient = client;
    ...
    // MStar Android Patch Begin
    mIsNWayPlayback = false;
    mGetkeyvalue = 0;
    mEnableMultiDecode = false;
    // MStar Android Patch End
...
{
    ALOGD("disconnect(%d) from pid %d", mConnId, mPid);
    ...
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
}
...
// MStar Android Patch Begin
static bool isFromSpecificProcess(const char *name)
{
    char buf[256];
...
static bool property_check_multiple_payerback_process(const char *key)
{
    char value[PROPERTY_VALUE_MAX];

    if (property_get(key, value, NULL)) {
        char *record_buffer = value;
        char sign_char = ',';
        char *ptr;
        for (;;) {
            ptr = strchr(record_buffer, sign_char);
            if (ptr) {
                *ptr = '\0';
                ALOGV("apk process is:%s\n", record_buffer);
                if (isFromSpecificProcess(record_buffer))
                    return true;
                record_buffer = ptr + 1;
            }
            else {
...
{
    ALOGD("[%d] setDataSource(%s)", mConnId, url);
    if (url == NULL)
...
status_t MediaPlayerService::Client::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    // now set data source
    mStatus = setDataSource_post(p, p->setDataSource(fd, offset, length));
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
    return mStatus;
}
...
status_t MediaPlayerService::Client::setDataSource(
    ...
    // create the right type of player
    ALOGD("[%d] %s IStreamSource", mConnId, __FUNCTION__);
    ...
    // now set data source
    mStatus = setDataSource_post(p, p->setDataSource(source));
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
    return mStatus;
}
...
status_t MediaPlayerService::Client::setDataSource(
    ...
    ALOGD("[%d] %s DataSource", mConnId, __FUNCTION__);
    ...
    // now set data source
    mStatus = setDataSource_post(p, p->setDataSource(dataSource));
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
    return mStatus;
}
...
status_t MediaPlayerService::Client::setDataSource(
        const sp<IMstarMMSource> &Source)
{
    player_type playerType;
    ST_MS_DATASOURCE_INFO stExtDataInfo;
    memset((void*)&stExtDataInfo,0,sizeof(ST_MS_DATASOURCE_INFO));
    Source->getDataSourceInfo(stExtDataInfo);
    if(stExtDataInfo.ePlayerType == E_MS_PLAYER_TYPE_MSTARPLAYER){
        playerType = MSTAR_PLAYER;
    }else{
    ...
    // now set data source
    ALOGD("[%d] %s IMstarMMSource", mConnId, __FUNCTION__);
...
{
    ALOGD("[%d] start", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    p->setLooping(mLoop);
    status_t ret = p->start();
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
    return ret;
}
...
{
    ALOGD("[%d] pause", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret = p->pause();
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
    return ret;
}
...
status_t MediaPlayerService::Client::getCurrentPosition(int *msec)
{
    ...
    if (ret == NO_ERROR) {
        ALOGD("[%d] getCurrentPosition = %d", mConnId, *msec);
    } else {
...
status_t MediaPlayerService::Client::getDuration(int *msec)
{
    ...
    if (ret == NO_ERROR) {
        ALOGD("[%d] getDuration = %d", mConnId, *msec);
    } else {
...
status_t MediaPlayerService::Client::setNextPlayer(const sp<IMediaPlayer>& player) {
    ALOGD("[%d] setNextPlayer %p", mConnId, player.get());
    Mutex::Autolock l(mLock);
...
{
    ALOGD("[%d] seekTo(%d, %d)", mConnId, msec, mode);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret = p->seekTo(msec, mode);
    ALOGD("[%d] %s done", mConnId, __FUNCTION__);
    return ret;
}
...
{
    ALOGD("[%d] setAudioStreamType(%d)", mConnId, type);
...
// MStar Android Patch Begin
#define DISABLE_TRACK -2
// MStar Android Patch End

status_t MediaPlayerService::Client::setVolume(float leftVolume, float rightVolume)
{
    ALOGD("[%d] setVolume(%f, %f)", mConnId, leftVolume, rightVolume);
    ...
    if (mIsNWayPlayback && p != 0 && p->playerType() == MST_PLAYER) {
        status_t ret;
        bool enableAudio = (leftVolume == 0 && rightVolume == 0) ? false : true;

        ALOGV("[%d][multi_player] setAudioTrack: %d (%f/%f)", mConnId, enableAudio, leftVolume, rightVolume);

        if (enableAudio)
          ret = p->setAudioTrack(0); // only support track 0 now
        else
          ret = p->setAudioTrack(DISABLE_TRACK);

        ALOGV("[%d][multi_player] ret: %d", mConnId, ret);
    }
    // MStar Android Patch End
...
{
    ALOGD("[%d] attachAuxEffect(%d)", mConnId, effectId);
    Mutex::Autolock l(mLock);
...
status_t MediaPlayerService::Client::setParameter(int key, const Parcel &request) {
    ...
    // MStar Android Patch Begin
    if (KEY_PARAMETER_SET_DUAL_DECODE_PIP == key || KEY_PARAMETER_SET_SEAMLESS_MODE == key)
        mGetkeyvalue = key;
    request.setDataPosition(0);

    // FIXME: force to disable mulitplayback function by default temporarily
    char multiplay_status[PROPERTY_VALUE_MAX];
    property_get("mstar.mwplaybackon", multiplay_status, "0");
    if (key == KEY_PARAMETER_SET_MULTIPLE_PLAYER &&
        !strcmp(multiplay_status, "0"))
    {
        ALOGE("Not Enable Multiplayback");
        return UNKNOWN_ERROR;
    }

    if (key == KEY_PARAMETER_SET_MULTIPLE_PLAYER)
        mIsNWayPlayback = true;
    // MStar Android Patch End
    switch (key) {
        ...
        void* cookie, int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    if (c != NULL) {
        ALOGD("[%d] notify (%p, %d, %d, %d)", client->mConnId, cookie, msg, ext1, ext2);
...
// MStar Android Patch Begin
bool MediaPlayerService::Client::setPlayMode(int speed)
{
    ALOGV("MediaPlayerService::Client::setPlayMode(%d)", speed);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    bool ret = p->setPlayMode(speed);
    if (ret == NO_ERROR) {
        ALOGV("speed = %d", speed);
    } else {
        ALOGE("setPlayMode returned %d", ret);
    }
    return ret;
}

int MediaPlayerService::Client::getPlayMode()
{
    ALOGV("MediaPlayerService::Client::getPlayMode");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    int ret = p->getPlayMode();
    ALOGD("MediaPlayerService::Client::getPlayMode, ret %d", ret);
    return ret;
}

int MediaPlayerService::Client::getPlayModePermille()
{
    ALOGV("MediaPlayerService::Client::getPlayModePermille");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return 0;

    int ret = p->getPlayModePermille();
    ALOGD("MediaPlayerService::Client::getPlayModePermille, ret %d", ret);
    return ret;
}

status_t MediaPlayerService::Client::setPlayModePermille(int speed_permille)
{
    ALOGV("MediaPlayerService::Client::setPlayModePermille(%d)", speed_permille);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return NO_INIT;

    status_t ret = p->setPlayModePermille(speed_permille);
    if (ret == NO_ERROR) {
        ALOGV("speed permille = %d", speed_permille);
    } else {
        ALOGE("setPlayModePermille returned %d", ret);
    }
    return ret;
}

status_t MediaPlayerService::Client::setAudioTrack(int track)
{
    ALOGV("MediaPlayerService::Client::setAudioTrack:%d",track);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    return p->setAudioTrack(track);
}

status_t MediaPlayerService::Client::setSubtitleTrack(int track)
{
    ALOGV("MediaPlayerService::Client::setSubtitleTrack");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
status_t MediaPlayerService::Client::setSubtitleDataSource(const char *url)
{
    ALOGV("MediaPlayerService::Client::setSubtitleDataSource");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    p->setSubtitleDataSource(url,NULL);
    return OK;
}

status_t MediaPlayerService::Client::offSubtitleTrack()
{
    ALOGV("MediaPlayerService::Client::offSubtitleTrack");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    p->onOffSubtitleTrack(false);
    return OK;
}

status_t MediaPlayerService::Client::onSubtitleTrack()
{
    ALOGV("MediaPlayerService::Client::onSubtitleTrack");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    p->onOffSubtitleTrack(true);
    return OK;
}

char* MediaPlayerService::Client::getSubtitleData(int *size)
{
    ALOGV("MediaPlayerService::Client::getSubtitleData");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return NULL;

    char *pStr = p->getSubtitleData(size);
    ALOGD("MediaPlayerService::Client::getSubtitleData, szBuffer: %d", *size);
    return pStr;
}

char * MediaPlayerService::Client::getAudioTrackStringData(int *size, int infoType)
{
    ALOGV("MediaPlayerService::Client::getAudioTrackStringData");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return NULL;

    char *pStr = p->getAudioTrackStringData(size, infoType);
    ALOGD("MediaPlayerService::Client::getAudioTrackStringData, szBuffer: %d", *size);
    return pStr;
}

status_t MediaPlayerService::Client::setSubtitleSurface(const sp<IGraphicBufferProducer>& surface)
{
    ALOGV("MediaPlayerService::Client::setSubtitleSurface");
    ALOGV("[%d] setSubtitleSurface(%p)", mConnId, surface.get());
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    p->setSubtitleSurface(surface);
    return  OK;
}

int MediaPlayerService::Client::setSubtitleSync(int timeMs)
{
    ALOGV("MediaPlayerService::Client::setSubtitleSync(%d)", timeMs);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    int ret = p->SubtitleSync(timeMs);
    if (ret == NO_ERROR) {
        ALOGV("timeMs = %d", timeMs);
    } else {
        ALOGE("setSubtitleSync returned %d", ret);
    }
    return ret;
}

void MediaPlayerService::Client::divx_SetTitle(uint32_t u32ID)
{
    ALOGV("MediaPlayerService::Client::divx_SetTitle");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return ;

    p->divx_SetTitle(u32ID);
}

status_t MediaPlayerService::Client::divx_GetTitle(uint32_t *pu32ID)
{
    ALOGV("MediaPlayerService::Client::divx_GetTitle");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    return p->divx_GetTitle(pu32ID);
}

void MediaPlayerService::Client::divx_SetEdition(uint32_t u32ID)
{
    ALOGV("MediaPlayerService::Client::divx_SetEdition");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return;

    p->divx_SetEdition(u32ID);
}

status_t MediaPlayerService::Client::divx_GetEdition(uint32_t *pu32ID)
{
    ALOGV("MediaPlayerService::Client::divx_GetEdition");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    return p->divx_GetEdition(pu32ID);
}

void MediaPlayerService::Client::divx_SetChapter(uint32_t u32ID)
{
    ALOGV("MediaPlayerService::Client::divx_SetChapter");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return ;

    p->divx_SetChapter(u32ID);
}

status_t MediaPlayerService::Client::divx_GetChapter(uint32_t *pu32ID)
{
    ALOGV("MediaPlayerService::Client::divx_GetChapter");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    return p->divx_GetChapter(pu32ID);
}

void MediaPlayerService::Client::divx_SetAutochapter(uint32_t u32ID)
{
    ALOGV("MediaPlayerService::Client::divx_SetAutochapter");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return;

    p->divx_SetAutochapter(u32ID);
}

status_t MediaPlayerService::Client::divx_GetAutochapter(uint32_t *pu32ID)
{
    ALOGV("MediaPlayerService::Client::divx_GetAutochapter");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->divx_GetAutochapter(pu32ID);
}

status_t MediaPlayerService::Client::divx_GetAutochapterTime(uint32_t u32ID, uint32_t *pu32IdTime)
{
    ALOGV("MediaPlayerService::Client::divx_GetAutochapterTime");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    return p->divx_GetAutochapterTime(u32ID, pu32IdTime);
}

status_t MediaPlayerService::Client::divx_GetChapterTime(uint32_t u32ID, uint32_t *pu32IdTime)
{
    ALOGV("MediaPlayerService::Client::divx_GetChapterTime");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    return p->divx_GetChapterTime(u32ID, pu32IdTime);
}

status_t MediaPlayerService::Client::divx_SetResumePlay(const Parcel &inResumeInfo){
    ALOGV("MediaPlayerService::Client::divx_SetResumePlay");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    status_t status = p->divx_SetResumePlay(inResumeInfo);
    if (status != OK) {
        ALOGE("divx_SetResumePlay failed %d", status);
        return status;
    }
    return status;
}

status_t MediaPlayerService::Client::divx_SetReplayFlag(bool isResumePlay){
    ALOGV("MediaPlayerService::Client::divx_SetReplayFlag(%d)", isResumePlay);
    sp<MediaPlayerBase> p = getPlayer();
    if (p != 0) return p->divx_SetReplayFlag(isResumePlay);
    return NO_ERROR;
}

status_t MediaPlayerService::Client::captureMovieThumbnail(sp<IMemory>& mem, Parcel *pstCaptureParam, int* pCapBufPitch)
{
    ALOGV("MediaPlayerService::Client::captureMovieThumbnail");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;

    status_t status = p->captureMovieThumbnail(mem, pstCaptureParam, pCapBufPitch);
    if (status != OK) {
        ALOGD("status != OK:%d",status);
        return status;
    }
    return OK;
}

status_t MediaPlayerService::Client::setVideoDisplayAspectRatio(int eRatio) {
    ALOGD("MediaPlayerService::Client::setVideoDisplayAspectRatio (%d)", eRatio);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setVideoDisplayAspectRatio(eRatio);
}

status_t MediaPlayerService::Client::getVideoPTS(int *msec)
{
    ALOGV("MediaPlayerService::Client::getVideoPTS");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret = p->getVideoPTS(msec);
    if (ret == NO_ERROR) {
        ALOGV("[%d] getVideoPTS = %d", mConnId, *msec);
    } else {
        ALOGE("getVideoPTS returned %d", ret);
    }
    return ret;
}

status_t MediaPlayerService::Client::setAVCodecChanged(bool /* bChanged */)
{
#if 0
    ALOGV("MediaPlayerService::Client::setAVCodecChanged");
    bAVCodecNotChanged = bChanged;
    if (bAVCodecNotChanged) {
        if (mClient.get() != NULL) {
             mClient->notify(MEDIA_PLAYBACK_COMPLETE, 0, 0, 0); //must
        }
    }
    LOGD("bAVCodecNotChanged=%d", bAVCodecNotChanged);
#endif
    return OK;
}
// MStar Android Patch End
        ...
        uint32_t suggestedFrameCount)
{
    ...
    if ((flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) != 0) {
        ...
        if (bufferCount != 0 && bufferCount < mMinBufferCount) {
            ALOGD("bufferCount (%d) increased to %d", bufferCount, mMinBufferCount);
...
status_t MediaPlayerService::AudioOutput::updateTrack() {
    ...
    ALOGD("updateTrack() DONE status %d", res);
    return res;
}
